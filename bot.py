...
async def do_verify(msg: Message, code: str, state: FSMContext | None = None):
    ...
    if ok:
        ...
        # Сохраняем pending-запись. mc_name = None пока не придёт verify_result.
        # Если аддон ответит ok=False — ptero_ws откатит запись через unlink_tg.
        # Если аддон ответит ok=True — ptero_ws запишет mc_name и пометит код использованным.
        ...
        await msg.answer(
            ...
            "• <b>+200 T</b> на баланс\n"
            "• <b>+10 Trust Score</b>\n\n"
            "Ваш MC-ник появится автоматически после подтверждения сервером.",
