...
# или нет — зависит от версии BDS. Ловим через строку в консоли.
...
class PteroConsoleWatcher:
    ...
    async def _handle_verify_result(self, data: dict):
        ...
        if ok and tg_id and mc_name:
            # Записываем mc_name который вернул аддон и помечаем код использованным
            user = self.db.find_by_tg_id(tg_id)
            if user:
                old = user.get('mc_name')
                if old != mc_name:
                    self.db.set_mc_name(tg_id, mc_name)
                    logger.info(f'[ptero_ws] verify_result: tg_id={tg_id} mc_name={mc_name}')
                # Помечаем код использованным (раньше не делали чтобы не блокировать до подтверждения)
                code = user.get('code', '')
                if code:
