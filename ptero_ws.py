                ...
                async for msg in ws:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        if ev_name == 'auth success':
                            ...
                            # Логируем ВСЕ остальные эвенты содержащие TG
                            raw = str(args)
                            if 'tg' in raw.lower():
                                logger.info(f'[ptero_ws] [DBG] ev={repr(ev_name)} raw={repr(raw[:300])}')

                        if ev_name == 'console output' and args:
                            line = args[0] if isinstance(args[0], str) else ''
                            line_clean = re.sub(r'\x1b\[[0-9;]*m', '', line)
                            # DEBUG: логируем ВСЕ строки содержащие Verify или warn
                            if any(x in line_clean.lower() for x in ['verify', 'warn', 'scripting']):
                                logger.info(f'[ptero_ws] [DBG] RAW: {repr(line_clean)}')
                            m_fail = PATTERN_VERIFY_FAIL_WARN.search(line_clean)
                            logger.debug(f'[ptero_ws] [DBG] FAIL_WARN match={m_fail} line={repr(line_clean[:80])}') if 'verify' in line_clean.lower() else None
