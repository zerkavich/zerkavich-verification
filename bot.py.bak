...
async def cb_menu_verify(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state("waiting_verify_code")
    await call.message.edit_text(
...
async def do_verify(msg: Message, code: str, state: FSMContext | None = None):
    """Выполняет верификацию. Используется и из /verify, и из меню."""
    uid      = str(msg.from_user.id)
    ...
    # Проверяем код ДО отправки на сервер — не привязываем TG к незарегистрированному аккаунту
    # Код считается использованным только после успешного ответа сервера
    ...
    if ok:
        # Только при успехе — сохраняем привязку и помечаем код использованным
        ...
        await msg.answer(
            ...
            "• <b>+200 T</b> на баланс\n"
            "• <b>+10 Trust Score</b>\n\n"
            "Ваш Telegram привязан к аккаунту. Ник появится после входа в игру.",
            reply_markup=main_menu_kb(is_verified=True),
            parse_mode="HTML"
        )
    else:
        # Ошибка — НЕ сохраняем ничего. TG остаётся свободным.
        await wait_msg.delete()
        await msg.answer(
            f"❌ <b>Код не принят сервером.</b>\n\n"
            f"Возможные причины:\n"
            f"• Код истёк (действует 30 минут)\n"
            f"• Код введён неверно\n"
            f"• Сервер недоступен\n\n"
            f"Получите новый код командой <code>.econ verify</code> в игре.\n"
            f"Или обратитесь в {APPEAL_URL}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
...
@dp.message(F.text, lambda msg: True)
async def handle_verify_code(msg: Message, state: FSMContext):
    current = await state.get_state()
    if current != "waiting_verify_code":
